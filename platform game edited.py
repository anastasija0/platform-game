...
class Player(object):
  ...
  def collide(self, tile_list):
    if self.visible == True:
      for i in tile_list:
        if self.hitbox[1] < i.y + i.height and self.hitbox[1] + self.hitbox[3] > i.y:
          if self.hitbox[0] + self.hitbox[2] > i.x and self.hitbox[0] < i.x + i.width:
              if self.right==True:
...
def load_data(map_data, enemies, coins):
...
def draw_tile(tile, screen):
        screen.blit(tile.image,(tile.x+scroll[0],tile.y+scroll[1]))
        
def load_tiles(map_data, tile_list, tiles):
  y = 0
  for layer in map_data:
    x = 0
    for tile in layer:
      if tile == '1':
        tile_list.append(Tile(x*32, y*32,tiles[0]))
      if tile == '2':
        tile_list.append(Tile(x*32, y*32,tiles[1]))
      if tile == '3':
        tile_list.append(Tile(x*32, y*32,tiles[2]))
      if tile == '4':
        tile_list.append(Tile(x*32, y*32,tiles[3]))
      if tile == '5':
        tile_list.append(Tile(x*32, y*32,tiles[4]))
      if tile == '6':
        tile_list.append(Tile(x*32, y*32,tiles[5]))
      if tile == '7':
        tile_list.append(Tile(x*32, y*32,tiles[6]))
      if tile == '8':
        tile_list.append(Tile(x*32, y*32,tiles[7]))
      if tile == '9':
        tile_list.append(Tile(x*32, y*32,tiles[8]))
      if tile == 'A':
        tile_list.append(Tile(x*32, y*32,tiles[9]))
      x += 1
    y += 1
    
        
load_data(map_data, enemies, coins)
scroll=[0,0]
load_tiles(map_data, tile_list, tiles)

def redrawGameWindow():
  screen.blit(bg, (0,0))
  for tile in tile_list:
      draw_tile(tile, screen)
...
while run:
  ...
  for event in pygame.event.get():
    if event.type == pygame.QUIT or p.visible == False:
      run = False
      pygame.quit()
  ...
  if keys[pygame.K_SPACE] and bLoop == 0:
    if p.left:
      ...
      direction=1
    if len(bullets)<6:
      bullets.append(Bullet(round(p.x+p.width //2),round(p.y+p.height //2), 3, (255,0,0), direction))
    bLoop = 1
